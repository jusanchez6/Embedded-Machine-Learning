...
with tf.device('/CPU:0'):
    ...
    model = models.Sequential([
        ...
        layers.Dropout(0.3),
        layers.Dense(16, activation='softmax')  # Asegúrate de que esto coincida con tu entrenamiento
    ])
    ...
    if len(X) == 0:
        raise RuntimeError("No se cargaron imágenes. Verifica la ruta ./test/")
...
peak_memory_MB = (mem_after - mem_before) / (1024 * 1024)  # Usar tracemalloc correctamente
...
print(f"Tiempo total: {total_time * 1000:.4f} ms")
